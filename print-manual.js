...
window.printKapachimManual=async function(){
  ...
  try{
    ...
    const content=sections.filter(s=>!['home','settings'].includes(s.type)).map(section=>{
      ...
      const docContent=section.type==='documents'?docs.map(doc=>`<div class="doc"><h3>${esc(doc.title)}</h3>${doc.text?`<p>${paragraphs(doc.text)}</p>`:''}${doc.body?`<p>${paragraphs(doc.body)}</p>`:''}${doc.editableText?`<p>${paragraphs(doc.editableText)}</p>`:''}${Number.isFinite(Number(doc.page))?`<figure><img src="${esc(asset(`assets/pages/page-${String(doc.page).padStart(2,'0')}.webp`))}" alt="${esc(doc.title)}"></figure>`:''}${Array.isArray(doc.steps)?doc.steps.map((step,i)=>`<div class="block"><h4>Βήμα ${i+1}</h4><p>${paragraphs(typeof step==='string'?step:JSON.stringify(step))}</p>${photoBlocks(`${doc.id}::step::${i}`)}</div>`).join(''):''}${noteBlocks(doc.id)}${photoBlocks(doc.id)}</div>`).join(''):'';
