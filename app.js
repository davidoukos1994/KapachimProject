...
function selectSection(s){
 ...
 currentSection=s;activeTab=s.type==='home'?'home':s.type==='documents'?'documents':s.type==='flow'?'flow':s.type==='settings'?'settings':'manual';document.body.classList.toggle('home-mode',s.type==='home');document.body.classList.toggle('manual-sector-mode',!s.type);$('#pageTitle').textContent=s.type==='home'?'Προσωπικό Manual':s.title;$('#breadcrumb').textContent=s.type==='home'?'Αρχική':s.type==='documents'?'Βιβλιοθήκη εγγράφων':s.type==='flow'?'Εργαλεία / Ροή Μονάδας':s.type==='settings'?'Εργαλεία / Ρυθμίσεις':'Τομείς / '+s.title;
 updateGlobalCompactNav();
 renderTabs();renderContent();buildNav($('#searchInput').value);updateEditButtons();closeSidebar()}
function goPreviousPage(){const last=pageHistory.pop();if(!last)return;navigatingHistory=true;try{if(last.docId){const d=docs.find(x=>x.id===last.docId);if(d)currentDoc=d}selectSection(last.sectionId==='home'?homeSection:sections.find(x=>x.id===last.sectionId)||homeSection)}finally{navigatingHistory=false}updateGlobalCompactNav();window.scrollTo({top:0,behavior:'smooth'});}
function updateGlobalCompactNav(){const nav=$('#globalCompactNav'),prev=$('#globalPrev'),next=$('#globalNext');if(!nav||!prev||!next)return;const isHome=currentSection.type==='home';nav.hidden=isHome;prev.disabled=pageHistory.length===0;prev.onclick=pageHistory.length?goPreviousPage:null;let ns=null;if(!currentSection.type)ns=nextManualSection();else if(currentSection.type==='flow')ns=firstManualSection();else if(currentSection.type==='documents')ns=null;else if(currentSection.type==='settings')ns=null;next.disabled=!ns;next.onclick=ns?()=>{selectSection(ns);window.scrollTo({top:0,behavior:'smooth'})}:null;}
...
function renderSystemSettings(c){
 ...
 c.innerHTML=`<div class="settings-page"><div class="section-head-row"><div><h2 class="section-title">Ρυθμίσεις Συστήματος</h2><p>Έλεγχος της κοινής online αποθήκης και της τελευταίας αποθήκευσης.</p></div><button class="btn primary run-health-check">Έλεγχος σύνδεσης</button></div><div class="status-grid"><section class="status-card"><span>Internet</span><strong>${navigator.onLine?'🟢 Συνδεδεμένο':'🔴 Χωρίς σύνδεση'}</strong></section><section class="status-card"><span>Supabase API</span><strong>${value(st.apiLabel,'Έλεγχος…')}</strong></section><section class="status-card"><span>Τρόπος λειτουργίας</span><strong>${value(st.realtimeLabel,'Απλή online αποθήκη')}</strong></section><section class="status-card"><span>Τελευταίος συγχρονισμός</span><strong>${value(st.lastSyncLabel,'Δεν έχει γίνει')}</strong></section><section class="status-card"><span>Έκδοση εφαρμογής</span><strong>v20</strong></section><section class="status-card"><span>Έκδοση βάσης</span><strong>${value(st.schemaVersion,'Schema v1')}</strong></section></div><section class="settings-details"><h3>Εκτύπωση εφαρμογής</h3><p>Δημιούργησε εκτυπώσιμη έκδοση όλων των τομέων, διαδικασιών, σημειώσεων και φωτογραφιών. Από την εκτύπωση μπορείς να επιλέξεις αποθήκευση ως PDF.</p><button type="button" class="btn primary print-manual">Εκτύπωση / PDF</button></section><section class="settings-details"><h3>Αντίγραφα ασφαλείας</h3><p>Κρατούνται online πριν από διαγραφές και περιοδικά πριν από αλλαγές. Η επαναφορά επηρεάζει όλους όσοι χρησιμοποιούν το ίδιο link.</p><button type="button" class="btn primary make-backup">Δημιουργία αντιγράφου τώρα</button><div class="backup-list" aria-live="polite">Φόρτωση αντιγράφων…</div></section><div class="settings-details"><h3>Διαγνωστικά</h3><dl><div><dt>Project URL</dt><dd>${value(st.projectUrl,'')}</dd></div><div><dt>Κατάσταση φόρτωσης</dt><dd>${value(st.loadState,'Αναμονή')}</dd></div><div><dt>Τελευταίο μήνυμα</dt><dd>${value(st.lastMessage,'—')}</dd></div><div><dt>Χρόνος απόκρισης</dt><dd>${value(st.latencyLabel,'—')}</dd></div></dl><p class="settings-note">Η εφαρμογή φορτώνει τα δεδομένα μία φορά όταν ανοίγει. Για να δεις νεότερες αλλαγές, πάτησε «Ανανέωση». Δεν γίνονται συνεχείς έλεγχοι στο παρασκήνιο.</p></div></div>`;
 c.querySelector('.run-health-check').onclick=async()=>{await window.runSupabaseHealthCheck?.(true);renderSystemSettings(c)};
 c.querySelector('.print-manual').onclick=()=>window.printKapachimManual();
 const list=c.querySelector('.backup-list');
 const refresh=async()=>{
   try{
     const rows=await window.listKapachimBackups();
     list.innerHTML=rows.length?rows.map(row=>`<div class="backup-row"><span><strong>${escapeHtml(new Date(row.created_at).toLocaleString('el-GR'))}</strong><small>${escapeHtml(row.reason)}</small></span><button type="button" class="btn secondary restore-backup" data-id="${escapeHtml(row.id)}">Επαναφορά</button></div>`).join(''):'Δεν υπάρχει ακόμη αντίγραφο.';
     list.querySelectorAll('.restore-backup').forEach(button=>button.onclick=async()=>{
       if(!confirm('Θα επαναφερθούν όλα τα κείμενα, οι διαδικασίες και οι φωτογραφίες σε αυτή την ημερομηνία για ΟΛΟΥΣ τους χρήστες. Θα κρατηθεί πρώτα αντίγραφο της τωρινής κατάστασης. Συνέχεια;'))return;
       button.disabled=true;
       try{await window.restoreKapachimBackup(button.dataset.id);alert('Η επαναφορά ολοκληρώθηκε.');renderSystemSettings(c)}catch(error){button.disabled=false;alert(`Η επαναφορά απέτυχε: ${error.message}`)}
     });
   }catch(error){list.textContent='Τα αντίγραφα δεν είναι ενεργά. Εκτέλεσε πρώτα το SUPABASE_BACKUP_V19.sql στο Supabase.'}
 };
 c.querySelector('.make-backup').onclick=async event=>{
   const button=event.currentTarget;button.disabled=true;
   try{await window.createKapachimBackup();await refresh();alert('Το αντίγραφο αποθηκεύτηκε online.')}catch(error){alert(`Δεν δημιουργήθηκε αντίγραφο: ${error.message}`)}finally{button.disabled=false}
 };
 refresh();
}
...
function renderManual(c){
 ...
 ${overviewImage?'<div class="original-badge custom">📷 Βασική εικόνα τομέα</div>':hasPage?'<div class="original-badge">🔒 Αυθεντικό περιεχόμενο PDF - μόνο για προβολή</div>':builtIns.length?'<div class="original-badge custom">📷 Ενσωματωμένο βοηθητικό υλικό</div>':'<div class="original-badge custom">✏️ Προσωπικός τομέας</div>'}
...
let dbGetNotes=s=>dbGetBySection('notes',s);let dbGetPhotos=s=>dbGetBySection('photos',s);async function dbDelete(store,id){const d=await openDB();return new Promise((res,rej)=>{const t=d.transaction(store,'readwrite');t.objectStore(store).delete(id);t.oncomplete=res;t.onerror=()=>rej(t.error)})}
