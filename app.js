...
const adminEnabled=()=>Boolean(window.isAdminMode?.());
...
window.refreshAdminVisibility=function(){document.body.classList.toggle('admin-mode',adminEnabled());document.querySelectorAll('.admin-only').forEach(el=>{el.hidden=!adminEnabled()});buildNav(document.querySelector('#searchInput')?.value||'');updateEditButtons();renderContent();};
